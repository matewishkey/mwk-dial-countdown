...
the brand assets in `com.matewishkey.dial-countdown.sdPlugin/imgs/`, the `mwk`
theme and the mark drawn in `src/render.ts` with your own, and change the
plugin UUID in `manifest.json` so your build does not collide with this one.
